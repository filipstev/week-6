  ...
  if (equation.includes('x') || equation.includes('/')) {
    equation.forEach((el) => {
      if (el === 'x') {
        ...
        if (equation[2] == 0) {
          alert('Ne mozemo da delimo sa 0');
          resetScreen();
          throw new Error('Greska: zabranjeno deljenje sa 0');
        }
        divide();
      }
    });
...
function multiply() {
  let sum =
    parseFloat(equation[equation.indexOf('x') - 1]) *
    parseFloat(equation[equation.indexOf('x') + 1]);
...
function divide() {
  let sum =
    parseFloat(equation[equation.indexOf('/') - 1]) /
    parseFloat(equation[equation.indexOf('/') + 1]);
  Number.isInteger(sum) ? sum : (sum = sum.toFixed(2));
...
function releaseKey(e) {
  if (range.value === '0') {
    if (e.target.classList.contains('num')) {
      e.target.style.boxShadow = '0 4px 0 0 hsl(28, 16%, 65%)';
    } else if (e.target === equalKey) {
      equalKey.style.boxShadow = '0 4px 0 0 hsl(6, 70%, 34%)';
    } else {
      e.target.style.boxShadow = '0 4px 0 0 hsl(224, 28%, 35%)';
    }
  } else if (range.value === '1') {
    if (e.target.classList.contains('num')) {
      e.target.style.boxShadow = '0 4px 0 0 hsl(35, 11%, 61%)';
    } else if (e.target === equalKey) {
      equalKey.style.boxShadow = '0 4px 0 0 hsl(25, 99%, 27%)';
    } else {
      e.target.style.boxShadow = '0 4px 0 0 hsl(185, 58%, 25%)';
    }
  } else {
    if (e.target.classList.contains('num')) {
      e.target.style.boxShadow = '0 4px 0 0 hsl(290, 70%, 36%)';
    } else if (e.target === equalKey) {
      equalKey.style.boxShadow = '0 4px 0 0 hsl(177, 92%, 70%)';
    } else {
      e.target.style.boxShadow = '0 4px 0 0 hsl(290, 70%, 36%)';
    }
  }
...
function changeTheme(e) {
  if (e.target.value === '0') {
    ...
    keys.forEach((key) => {
      ...
      key.style.boxShadow = '0 4px 0 0 hsl(185, 58%, 25%)';
      key.firstElementChild.style.color = 'white';
    });
    numKeys.forEach((key) => {
      key.style.backgroundColor = 'hsl(45, 7%, 89%)';
      key.style.boxShadow = '0 4px 0 0 hsl(35, 11%, 61%)';
    ...
    keys.forEach((key) => {
      ...
      key.style.boxShadow = '0 4px 0 0 hsl(285, 91%, 52%)';
      key.firstElementChild.style.color = 'white';
    });
    numKeys.forEach((key) => {
      key.style.backgroundColor = 'hsl(268, 47%, 21%)';
      key.style.boxShadow = '0 4px 0 0 hsl(290, 70%, 36%)';
